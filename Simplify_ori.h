...
//					&uv.x,&uv.y,&uv.z)==3)
//				{
//					uvs.push_back(uv);               // 每个点有一个 vec3f 的 uv, 所有 uv 构成 uvs, 相当于 顶点x, y, z
//				}
//			}
//			else if ( line[0] == 'v' )
//			{
...
//					has_uv = true;
//				}
//				else if (sscanf(line, "f %d/%d %d/%d %d/%d",// Add Support for v/vt only meshes
...
//						uvMap.push_back(indices);							// uvMap 即 uv 的索引  uvMap[i] 表示第 i 个三角形的 uv 索引, uvMap[i][j] 表示第 i 个三角形的第 j 个顶点的 uv 的索引, 指向一个 vec3f
//						t.attr |= TEXCOORD;
//					}
//
//					t.material = material;
